        ...
        return (
            <div key={this.props.id} className={styles.answersOption}>
                <div className={styles.answersRadio}>
                    ...
                    <label htmlFor={`answer-checkbox${this.props.id}`} className={styles.answersRadioLabel}>
                        {this.props.title}
                    </label>
                </div>
                <div className={styles.answersFieldWrapper}>
                    <div className={styles.answersFeedback + ' ' + additionallyСlass}>
                        <label className={styles.answersFeedbackTitle} htmlFor={`feedback-selected${this.props.id}`}>
                            Selected feedback (specific)
                        </label>
                        <textarea rows={1} className={styles.answersFeedbackField}
                                  id={`feedback-selected${this.props.id}`}
                                  placeholder='Enter feedback for when the choice is selected'/>

                        <label className={styles.answersFeedbackTitle} htmlFor={`feedback-unselected${this.props.id}`}>
                    ...
                    <div className={styles.answersItemWrapper}>
                        <label className={styles.sr} htmlFor={`answer-multi${this.props.id}`}>{placeholderText}</label>
                        <textarea rows={1} className={styles.answersItem} id={`answer-multi${this.props.id}`}
